...
import { View2D, getImageData, loadImageData, View3D } from '../vtk-viewport';
...
function createCT3dPipeline(imageData, ctTransferFunctionPresetId) {
  ...
  const preset = presets.find((preset) => preset.id === ctTransferFunctionPresetId);
...
class MPR extends Component {
  ...
  async getImageIds() {
    ...
    const response = await fetch(
      `${Config.hostname}:${Config.port}/${Config.qido}/studies/${this.props.studyUid}/series/${this.props.seriesUid}/instances`
    );
  ...
  async componentDidMount() {
    ...
    Promise.all(promises).then(
      () => {
        ...
        const ctVolVR = createCT3dPipeline(imageDataObject.vtkImageData, this.state.ctTransferFunctionPresetId);
              ...
              {this.state.volumes && (
                <>
                  <div className="col-xs-12 col-sm-3">
                    <View2D volumes={this.state.volumes} onCreated={this.storeApi('AXIAL')} />
                  </div>
                  <div className="col-xs-12 col-sm-3">
                    <View2D volumes={this.state.volumes} onCreated={this.storeApi('SAGITTAL')} />
                  </div>
                  <div className="col-xs-12 col-sm-3">
                    <View2D volumes={this.state.volumes} onCreated={this.storeApi('CORONAL')} />
                  </div>
                  <div className="col-xs-12 col-sm-3">
                    <View3D volumes={this.state.volumeRenderingVolumes} onCreated={this.saveApiReference} />
                  </div>
                </>
              )}
            </div>
